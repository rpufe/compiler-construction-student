    ...
    match e:
        ...
        case Call(id, args):
            ...
            match id:
                ...
                case Name(var, Var()):
                    params:List[ty]=[a.ty.ty for a in atomArgs if a.ty is NotVoid]
                    return atomic(needAtomic, atom.Call(atom.CallTargetIndirect(var, params, t), atomArgs, t), utils.flatten(tmps), ctx)
                case Call(fun, args, _):
                    return transExp(fun, False, ctx)
                case _:
                    raise ValueError(f'type still None after type-checking. Expression: {e}')
        case UnOp(op, sub):
